...
public class ManagerService {
	...
	@Transactional
	public ManagerSaveResponseDto saveManager(AuthUser authUser, long todoId,
											  ManagerSaveRequestDto managerSaveRequest) {

		User user = User.fromAuthUser(authUser);
		Todo todo = todoRepository.findById(todoId).orElseThrow(() -> new InvalidRequestException("Todo not found"));

		if (todo.getUser() == null || !ObjectUtils.nullSafeEquals(user.getId(), todo.getUser().getId())) {
			throw new InvalidRequestException("담당자를 등록할 권한이 없습니다. 일정 작성자만 담당자를 등록할 수 있습니다.");
		}

		User managerUser = userRepository.findById(managerSaveRequest.getManagerUserId())
			.orElseThrow(() -> new InvalidRequestException("등록하려고 하는 담당자 유저가 존재하지 않습니다."));

		if (managerRepository.existsByTodoIdAndUserId(todoId, managerUser.getId())) {
			throw new InvalidRequestException("이미 등록된 담당자 유저입니다.");
		}
		...
		return new ManagerSaveResponseDto(
			...
			new UserResponseDto(managerUser)
		);
	}

	public List<ManagerResponseDto> findManagers(long todoId) {
